...
def render():
    ...
    with t1:
        ...
        st.caption("アプリケーションへの改善要望や不具合報告を投稿できます。開発者ログに即座に反映されます。")
        ...
        with st.form("feedback_input_form", clear_on_submit=True):
            ...
            submitted = st.form_submit_button("✨ フィードバックを送信する", type="primary")
            ...
            if submitted:
                if not title or not content:
                    ...
                    st.success(f"🎉 フィードバック 「{title}」 を正常に登録・送信しました！「ログ一覧・管理」タブで確認できます。")
                    st.toast("🎉 フィードバックを登録しました！")
