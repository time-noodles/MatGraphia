...
from ui.helpers import (
    ...
    build_event_label,build_sample_label,build_literature_label,build_measurement_label
)


def _load_material_plugin(plugin_name):
    """materialsプラグインを動的に読み込む"""
...
def render():
    ...
    # --------------------------------------------------
        ...
        smp_options={"(なし)":None}
        for s in samples:
            smp_options[f"{s['human_id']} ({s['form']})"]=s["sample_id"]

        evt_options={"(なし)":None}
        for e in rows:
            evt_options[build_event_label(e)]=e["event_id"]

        target_input_sample_ids=[]
        target_reference_literature_ids=[]
        target_reference_event_ids=[]
        try:
            target_input_sample_ids=json.loads(target.get("input_sample_ids") or "[]")
        except Exception:
            target_input_sample_ids=[]
        try:
            target_reference_literature_ids=json.loads(target.get("reference_literature_ids") or "[]")
        except Exception:
            target_reference_literature_ids=[]
        try:
            target_reference_event_ids=json.loads(target.get("reference_event_ids") or "[]")
        except Exception:
            target_reference_event_ids=[]

        if not target_input_sample_ids and target.get("input_sample_id"):
            target_input_sample_ids=[target.get("input_sample_id")]
        if not target_reference_literature_ids and target.get("reference_literature_id"):
            target_reference_literature_ids=[target.get("reference_literature_id")]
        if not target_reference_event_ids and target.get("reference_event_id"):
            target_reference_event_ids=[target.get("reference_event_id")]
            ...
            if st.button("コピーして新規作成",key=f"copy_btn_evt_{sel_id}"):
                try:
                    ...
                    new_evt=Event(
                        ...
                        input_sample_id=target.get("input_sample_id"),
                        reference_event_id=target.get("reference_event_id"),
    ...
    # --------------------------------------------------
        ...
        events_dict={e["event_id"]:e for e in events_raw}
    ...
    # --------------------------------------------------
        ...
        if not isinstance(extracted_preview,dict):
            extracted_preview={}
        conditions_preview=_json_or_raw(target.get("conditions") or "{}")
        if not isinstance(conditions_preview,dict):
            conditions_preview={}
        ...
        if str(target.get("measurement_type") or "").upper()=="XRD":
            if xrd_sim_preview:
                st.info(
                    f"XRD simulation: mode={xrd_sim_preview.get('mode')} / target={xrd_sim_preview.get('target')} / peaks={xrd_sim_preview.get('peak_count')}"
                )
            else:
    ...
    # --------------------------------------------------
        ...
        if cif_abs:
            st.caption(f"CIF absolute path: {cif_abs}")

        # ================================================================
        # CIF 結晶構造プレビュー + XRDシミュレーション (pymatgen + py3Dmol)
        # ================================================================
        if cif_abs:
            try:
                with open(cif_abs,"rb") as f:
                    cif_bytes_raw=f.read()
            ...
            if cif_bytes:
                ...
                if xrd_plugin and hasattr(xrd_plugin,"simulate_xrd_for_cif_bytes"):
                    ...
                    try:
                        ...
                        if sim_result:
                            if png_bytes:
                                st.image(png_bytes,caption=f"XRD simulated pattern ({sim_result.get('mode')})",use_container_width=True)
                            if plot_err:
                                st.warning(plot_err)
        ...
        curr_impurity_ids_view=_json_or_raw(target.get("impurity_material_ids") or "[]")
        if not isinstance(curr_impurity_ids_view,list):
            curr_impurity_ids_view=[]
        curr_polymorph_ids_view=_json_or_raw(target.get("polymorph_material_ids") or "[]")
        if not isinstance(curr_polymorph_ids_view,list):
            curr_polymorph_ids_view=[]
        ...
        with st.form("form_edit_mat"):
            ...
            for m in all_materials:
                if m["material_id"]==sel_id:
                    continue
                mat_options[f"{m['name']} #{m['material_id'][:4]}"]=m["material_id"]

            curr_impurity_ids=_json_or_raw(target.get("impurity_material_ids") or "[]")
            if not isinstance(curr_impurity_ids,list):
                curr_impurity_ids=[]
            curr_polymorph_ids=_json_or_raw(target.get("polymorph_material_ids") or "[]")
            if not isinstance(curr_polymorph_ids,list):
                curr_polymorph_ids=[]
