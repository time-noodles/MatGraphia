...
from ui.helpers import get_sample_ancestry,build_sample_label
...
def _render_xrd_upload_preview(uploaded_bytes,file_name,selected_sample_id,samples,events_dict,materials):
    """アップロード直後にXRDデータのプレビューグラフを描画する（シミュレーション比較付き）"""
    import io as _io
    ...
    if exp_tth is None or len(exp_tth)==0:
        return
    ...
    # 選択サンプルからMaterialの保存済みシミュレーション結果を取得
    sim_results_list=[]
    ...
    # シミュレーションがある場合: render_comparison_xrd_plot_pngで比較グラフ描画
    ...
    try:
        ...
        if create_symple_plots is None or plt is None:
            ...
            if ey_max>0:
                exp_int_norm=exp_int_norm/ey_max
...
def render():
    ...
    # XRDデータアップロード時の即時プレビュー（フォーム外で描画、シミュレーション比較付き）
    ...
    with st.form("measurement_form"):
        ...
        for msg in plugin_info_msgs:
            st.success(msg)
        for msg in plugin_warn_msgs:
            st.warning(msg)
            ...
            try:
                ...
                if measurement_type=="XRD" and plugin:
                    try:
                        ...
                        # 対応するMaterialから保存済みシミュレーション結果を取得（再計算なし）
                        sim_results_list=[]
                        main_material=None
                        ...
                        # シミュレーション結果をextracted_featuresに保存
                        if sim_results_list:
