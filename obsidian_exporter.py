...
class ObsidianExporter:
    ...
    def _format_lit(self,r:Dict)->str:
        ...
        tags = ["Literature", f"Literature/{sanitize_tag(r['literature_type'])}", "status/draft" if is_draft else "status/main"]
        if r.get("venue"):tags.append(f"Venue/{sanitize_tag(r['venue'])}")
        fm={"id":r["literature_id"],"title":r["title"],"authors":r["authors"],"doi":r["doi"],"year":r["publication_year"],"pages":r.get("pages"),"date":r.get("publication_year"),"is_draft":is_draft,"tags":tags}
        body=f"# {r['title'] or r['doi']}\n\n"
        pages_str=f", pp. {r['pages']}" if r.get("pages") else ""
        vol_str=f", Vol. {r['volume']}" if r.get("volume") else ""
        body+=f"- Status: {'📝 下書き (Draft)' if is_draft else '✨ 本登録 (Main)'}\n- Authors: {r['authors']}\n- DOI: {r['doi']}\n- Venue: {r['venue']} ({r['publication_year']}){vol_str}{pages_str}\n\n"
    ...
    def _format_evt(self,r:Dict)->str:
        is_draft = bool(r.get("is_draft"))
        tags = ["Event", f"Event/{sanitize_tag(r['event_type'])}", f"Material/{sanitize_tag(r['target_material'])}", "status/draft" if is_draft else "status/main"]
        if r.get("project_id"):tags.append(f"Project/{sanitize_tag(r['project_id'])}")
        fm={"id":r["event_id"],"type":r["event_type"],"material":r["target_material"],"date":r["created_at"][:10],"is_draft":is_draft,"tags":tags}
    ...
    def _format_smp(self,r:Dict)->str:
        is_draft = bool(r.get("is_draft"))
        tags = ["Sample", f"Sample/Form/{sanitize_tag(r['form'])}", "status/draft" if is_draft else "status/main"]
        fm={"id":r["sample_id"],"human_id":r["human_id"],"form":r["form"],"date":r["created_at"][:10],"is_draft":is_draft,"tags":tags}
        body=f"# Sample: {r['human_id']}\n\n- Status: {'📝 下書き (Draft)' if is_draft else '✨ 本登録 (Main)'}\n- Form: {r['form']}\n- Location: {r['location'] or 'Unknown'}\n\n"
        body+=f"> [!note] **Remarks (備考)**\n> {r['remarks'] or 'None'}\n\n"
        body+="## Relationships\n"
    ...
    def _format_msr(self,r:Dict)->str:
        is_draft = bool(r.get("is_draft"))
        tags = ["Measurement", f"Measurement/{sanitize_tag(r['measurement_type'])}", "status/draft" if is_draft else "status/main"]
        fm={"id":r["measurement_id"],"type":r["measurement_type"],"operator":r["operator"],"date":r["measured_at"][:10],"is_draft":is_draft,"tags":tags}
